    ...
    /* if this is a repeat observation of a sky field, compute the ra and
       ...
       observation and hour angle of the pending observation */
        ...
        if(TRACKING_CORRECTIONS_ON){
            ...
            if(verbose){
                fprintf(stderr,"observe_next_field:  tracking corrections: HA = %7.3f, Dec = %7.3f, Ra and Dec rates: %7.3f %7.3f\n",
...
/*
fprintf(stderr,"HA = %7.3f,  expt = %7.3f,  LONG_EXPTIME = %7.3f\n",
...
*/
  ...
  for(n=1;n<=num_exposures;n++){
    *dt=expt+EXPOSURE_OVERHEAD;
    ...
    fprintf(stderr,
            "UT: %10.6f JD: %12.6f Exposed field : %d  RA: %9.6f  Dec: %9.5f n_done: %d n_wanted: %d time_left : %7.3f jd_next : %7.3f  description: %s\n",
  ...
  *dt=num_exposures*(expt+EXPOSURE_OVERHEAD);
    ...
    if(f->shutter!=DARK_CODE&&f->shutter!=DOME_FLAT_CODE){ 
        ...
        if(point_telescope(ra,dec,ra_rate,dec_rate)!=0){
           fprintf(stderr,"observe_next_field: ERROR pointing telescope to %7.3f %7.3f\n",
		ra,dec);
           return(-1);
        }
        ...
        if(verbose){
           fprintf(stderr,
             "observe_next_field: done pointing telescope in %d sec\n",
              t2.tv_sec-t1.tv_sec);
           fflush(stderr);
        }
    }
    ...
    if(filter_name_ptr!=0){
       sprintf(tel_status->filter_string,filter_name_ptr);
    }
    ...
    if(ut_prev<0.0||dt1>CLEAR_INTERVAL){
        if(ut_prev>0.0){
            fprintf(stderr,
		"observe_next_field: %7.3f minutes since last readout. Clearing camera..\n",
	        dt1*60.0);
        }
        else{
            fprintf(stderr,
    ...
    for(n=1;n<=num_exposures;n++){
         ...
         gettimeofday(&t2,NULL);
         if(verbose){
           fprintf(stderr,"observe_next_field: Exposure complete in %d sec, ut = %9.6f\n",
				...
				double jd, int bad_weather)
{
     ...
     /* if the pair to the previous fields is doable,
        this is first pick */
     ...
     if(f_prev!=NULL&&paired_fields(f_next,f_prev)&&f_next->doable){
	    ...
	    return(i_prev+1);
          ...
          if(f_next->status==READY_STATUS){
             ...
             fprintf(stderr,
                  "get_next_field: field %d is paired with previous field %d but is neither ready nor too late\n",
                  i_prev+1,i_prev);
             return(i_prev+1);
        }
     }
     ...
     /* If there are MUST-DO fields with TOO_LATE_STATUS, choose the one
        ...
        if(i_min<0){
           if(verbose)fprintf(stderr,"get_next_field: No MUST-DO late fields to shorten\n");
           return(-1);
        }
     ...
     /* Otherwise, if there are MUST_DO fields with READY_STATUS, 
        choose the one that has least time left to complete the 
	required observations */       


     else if(n_ready_must_do>0){
     ...
     /* Otherwise, if there are fields with READY_STATUS, choose the one 
        that has least time left to complete the required observations */

     else if(n_ready>0){

      if(verbose) {
          fprintf(stderr,"get_next_field: No must-do fields ready or late. %d ready fields\n",n_ready);
       }
     ...
     /* If there are no observable fields ready to observe,  but there are
        ...
        if(verbose) {
          fprintf(stderr,"get_next_field: No fields ready and no MUST-DO fields late. %d late fields\n",n_late);
